...
describe('slug', () => {
  ...
  describe('when given a null value', () => {
    it('should throw a validation error when required', () => {
      expect(() => {
        Joi.attempt(null, Joi.slug().required());
      }).to.throw(ValidationError, '"value" is required');
    });

    it('should return null when not required', () => {
      const result = Joi.attempt(null, Joi.slug());
      expect(result).to.be.null;
    });

    it('should return a default value when specified', () => {
      const result = Joi.attempt(null, Joi.slug().default('foo'));
      expect(result).to.eq('foo');
    });
  });
  ...
  describe('when given an empty string value', () => {
    it('should throw a validation error when required', () => {
      expect(() => {
        Joi.attempt('', Joi.slug().required());
      }).to.throw(ValidationError, '"value" is required');
    });

    it('should return null when not required', () => {
      const result = Joi.attempt('', Joi.slug());
      expect(result).to.be.null;
    });

    it('should return a default value when specified', () => {
      const result = Joi.attempt('', Joi.slug().default('foo'));
      expect(result).to.eq('foo');
    });
  });
