...
describe('sluggify', () => {
  ...
  it('should convert & to and', () => {
    expect(sluggify('foo & bar')).to.eq('foo-and-bar');
    expect(sluggify('foo&bar')).to.eq('fooandbar');
    expect(sluggify('& foo  &  bar &')).to.eq('and-foo-and-bar-and');
  });
  it('should convert underscores to dashes', () => {
    expect(sluggify('foo_bar')).to.eq('foo-bar');
    expect(sluggify('_foo_  _ __bar_')).to.eq('foo-bar');
    expect(sluggify('____')).to.be.null;
  });
});
