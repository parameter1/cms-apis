...
describe('prepare-document', () => {
  ...
  it('should consistently return the same object regardless of key order', () => {
    const expected = {
      _connection: {
        ancestors: [
          { _id: 67013, depth: 2 },
      ...
      c: 3,
      d: [1, 3, 4],
      e: [{ a: 1, b: 1 }, { a: 1, b: 1 }, { a: 2, b: -1 }, { a: 3, b: -2 }],
    };
    ...
    const r1 = prepare(v1, ['c', 'a', 'e', 'b', 'd']);
    expect(r1).to.deep.equal(expected);

    const r2 = prepare(v2, ['e', 'b.c', 'b.a', 'a', 'c', 'd']);
    expect(r2).to.deep.equal(expected);
  });
});
